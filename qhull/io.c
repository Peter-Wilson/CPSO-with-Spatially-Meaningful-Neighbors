...
/*-----------------------------------------
-printdegeneratehelp- prints descriptive message
*/
void printdegeneratehelp(FILE *fp) {
  static boolT onetime= True;

  if (onetime) {
    onetime= False;
    fprintf(fp, "\nqhull output is not convex.\nThis is probably due to degenerate input data, e.g. dimension+1 coplanar points.\nIf you want output anyway, use the 'o' flag.\nVersion 2.0 will handle degenerate input.\n\n");
  }
} /* printdegeneratehelp */
...
void printhashtable(FILE *fp) {
  ...
  for (i=0; i< HASHTABLEsize; i++) {
    if (hash_table[i])
      fprintf (fp, "hash %d\n", i);
    for (hash=hash_table[i]; hash; hash=hash->next) {
      printfacet (fp, hash->facet);
    }
  }
} /* printhashtable */
...
void printoff4 (FILE *fp, facetT *facetlist, pointT *points, int numpoints) {
  ...
  FORALLfacet_(facetlist) {
	...
	continue;
      vertices= vertexintersect(facet->vertices, neighbor->vertices, 
				&dummy, &k);
      numridges++;
